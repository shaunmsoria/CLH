...
launch_xvfb() {
    ...
    Xvfb ${DISPLAY} -screen ${screen} ${resolution} +extension RANDR &
    local loopCount=0
...
launch_window_manager() {
    ...
    # Start and wait for either awesome to be fully up or we hit the timeout.
    awesome &
    local loopCount=0
    ...
    do
        ...
        if [ ${loopCount} -gt ${timeout} ]
        then
            echo "${G_LOG_E} awesome failed to start."
            exit 1
        fi
    done
}
